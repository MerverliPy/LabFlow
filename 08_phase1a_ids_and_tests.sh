...
cat > packages/memory/package.json <<'EOF'
{
  ...
  "scripts": {
    ...
    "test": "node --test test/store.test.mjs && node --input-type=module -e \"import('./src/index.mjs').then((m) => { const dir = m.ensureStateDir(); if (!dir.endsWith('.labflow')) process.exit(1); console.log('memory exports ok'); })\""
  },
  "repository": {
    "type": "git",
