...
for (const name of stable)
  if (!manifest.stableCommands.includes(name))
    fail(`missing stable command: ${name}`);
for (const name of stable) {
  if (!manifest.commandStatus?.[name]) fail(`missing command status: ${name}`);
  if (!allowedStatuses.has(manifest.commandStatus[name]))
    fail(`invalid command status: ${name}`);
  if (!manifest.commandSummary?.[name])
    fail(`missing command summary: ${name}`);
}
for (const name of skills) {
  if (!manifest.skills.includes(name)) fail(`missing skill: ${name}`);
  if (!fs.existsSync(path.join(root, '.claude', 'skills', name, 'SKILL.md')))
    fail(`missing skill file: ${name}`);
}
for (const name of agents) {
  if (!manifest.subagents.includes(name)) fail(`missing subagent: ${name}`);
  if (!fs.existsSync(path.join(root, '.claude', 'agents', `${name}.md`)))
    fail(`missing subagent file: ${name}`);
}
for (const doc of requiredDocs) {
  if (!fs.existsSync(doc))
    fail(`missing required doc: ${path.relative(root, doc)}`);
}
if (manifest.identity.productName !== 'LabFlow')
  fail('identity drift: product');
if (manifest.identity.packageName !== 'labflow')
  fail('identity drift: package');
if (manifest.identity.cliName !== 'labflow') fail('identity drift: cli');
if (manifest.skills.length !== 8) fail('skill cap drift');
